...
const SESSION_KEY = "paketlager_session_v7";
const STORAGE_KEY = "paketlager_packages_v7";
...
function canAutoSave() {
  return !!(els.storageNumber.value && hasMeaningfulData());
}

function autoSaveCurrentPackage(reasonText) {
  if (!canAutoSave()) {
    setStatus(els.acceptStatus, reasonText || "Scan abgeschlossen.");
    return;
  }
  ...
  try {
    saveLock = true;
    saveCurrentPackage();
    setStatus(els.acceptStatus, reasonText || "Paket automatisch gespeichert.");
  } catch (error) {
    setStatus(els.acceptStatus, `Auto-Speichern fehlgeschlagen: ${error.message || error}`);
  } finally {
    setTimeout(() => {
      saveLock = false;
    }, 300);
...
async function processAddressImage(file) {
  ...
  autoSaveCurrentPackage(
    `Adressscan abgeschlossen. Paket automatisch gespeichert unter Lagernummer ${els.storageNumber.value || "-"}.`
  );
}
...
async function startBarcodeScan() {
  ...
  await html5Qr.start(
    ...
    decodedText => {
      ...
      autoSaveCurrentPackage(
        `Barcode erkannt und Paket automatisch gespeichert: ${decodedText}`
      );
    },
    () => {}
  );
  ...
  els.saveBtn.addEventListener("click", () => {
    try {
      if (!hasMeaningfulData()) {
        setStatus(els.acceptStatus, "Keine Daten zum Speichern vorhanden.");
        return;
      }

      saveCurrentPackage();
      setStatus(els.acceptStatus, "Paket gespeichert.");
    } catch (error) {
